#!/usr/bin/env python3
"""
PocketFence Universal Filter - Lightweight Performance Test
Cross-platform proxy filter performance validation (no external dependencies)
"""
...
class FilterPerformanceTestLite:
    def __init__(self, test_duration=30, concurrent_connections=50, output_path=None):
        self.test_duration = test_duration
        self.concurrent_connections = concurrent_connections
        self.output_path = output_path or f"filter-performance-lite-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt"
        self.test_results = {}
        self.test_log = []
    ...
    def start_filter(self):
        ...
        while time.time() - start_time < max_wait:
            try:
                # Simple check - try to connect to the proxy
                proxy_handler = urllib.request.ProxyHandler({'http': 'http://127.0.0.1:8888'})
                opener = urllib.request.build_opener(proxy_handler)
                opener.addheaders = [('User-agent', 'PocketFence-Test')]
    ...
    def measure_startup_performance(self):
        ...
        for i in range(1, 6):
            ...
            while time.time() < timeout:
                try:
                    proxy_handler = urllib.request.ProxyHandler({'http': 'http://127.0.0.1:8888'})
                    opener = urllib.request.build_opener(proxy_handler)
                    response = opener.open("http://example.com", timeout=0.5)
                    if response:
        ...
        if startup_times:
            self.test_results['avg_startup_time'] = sum(startup_times) / len(startup_times)
    ...
    def measure_filtering_performance(self):
        ...
        try:
            ...
            while time.time() < end_time:
                for url in self.test_urls:
                    try:
                        ...
                        # Check if blocked
                        if "PocketFence" in content and "Content Blocked" in content:
                            blocked_count += 1
            ...
            if response_times:
                self.test_results['avg_response_time'] = sum(response_times) / len(response_times)
        ...
        try:
            ...
            for url, should_block, description in self.test_cases:
                try:
                    ...
                    is_blocked = "PocketFence" in content and "Content Blocked" in content
    ...
    def calculate_performance_grade(self):
        ...
        # Performance score (over 10 req/s = 40 points, combining memory score since we can't measure it)
    ...
    def generate_report(self):
        ...
        report = f"""🚀 PocketFence Universal Filter Performance Report (Lite)
...
🧠 MEMORY USAGE:
  Memory monitoring not available (install psutil for detailed memory stats)
...
🏆 PERFORMANCE GRADE:
  ...
  Note: Memory tests skipped (install psutil for full scoring)
...
"""
    ...
    def run_all_tests(self):
        ...
        print("🧪 PocketFence Universal Filter Performance Test (Lite)")
        ...
        self.log_test("Note: Using lite version - no external dependencies required", "cyan")
        print()
...
def main():
    parser = argparse.ArgumentParser(description="PocketFence Universal Filter Performance Test (Lite)")
    ...
    tester = FilterPerformanceTestLite(
